...
class BaseDA(object):
    ...
    def getbyidscan(self, controllerName,barcode, serializer=False):
        ...
        if uid:
            ...
            response =models.execute_kw(db, uid, password, controllerName, 'search_read', [['|','|','|',['name','=',barcode],['origin','=',barcode],['transfer_id.name','=',barcode],['consume_id.name','=',barcode]]], {'fields': fields_list})
            if not response:
